...
export function GlowingStarsDescription({ className, children }: { className?: string; children?: React.ReactNode }) {
    return <p className={cn("text-base text-card-foreground max-w-[16rem]", className)}>{children}</p>;
}

export function GlowingStarsTitle({ className, children }: { className?: string; children?: React.ReactNode }) {
    return <h2 className={cn("font-bold text-xl text-card-foreground", className)}>{children}</h2>;
}
